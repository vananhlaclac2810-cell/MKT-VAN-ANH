...
**AUTOPILOT (override) — 0 checkpoint, 0 preview:** Theo mục AUTOPILOT trong `agents/videoeditor/CLAUDE.md`. MP3 tự duyệt, visual plan tự duyệt (`auto_scenes=true`), render thẳng (KHÔNG Studio gate). Thiếu info thì tự chọn default. Render xong gửi MP4 Telegram.
...
Nếu MP3 fail/empty → rerun TTS 1 lần; vẫn fail thì báo lỗi lên Telegram.
...
### Step 5 — Render MP4 + gửi Telegram (AUTOPILOT — KHÔNG preview)
...
Sau khi có MP4, format report + gửi file (≤ 50 MB):
...
[SEND_FILE:/absolute/path/workspace/content/YYYY-MM-DD/<slug>/<slug>.mp4|<slug> — keynote 16:9]
```
