...
> **Canvas note (HeyGen variant):** scene canvas là **1200×1080** (slide pane), KHÔNG 1920×1080. Khi PIP active, master tween slide-mount width về 1920 — scene tự stretch (CSS đã có `width: 100% !important` ở inner). Patterns dưới đây vẫn áp dụng nhưng adjust:
> - Hero text 80-110px (NOT 124px — pane hẹp hơn)
> - Particle bokeh field 60-90 dots (NOT 80-120)
> - Card grid spacing chật hơn — `card-grid-3x2` có thể tight, prefer `card-grid-2x2` hoặc `card-row-4` cho 4 items
> - Image-feature frame max 1080px wide
> - KHÔNG layout element ở right edge (>1100px) — sẽ overlap avatar frame khi tween về SPLIT 1200 width
