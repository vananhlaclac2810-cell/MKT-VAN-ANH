...
Dedicated **16:9 (1920×1080) landscape editor** — input là 1 folder workspace có sẵn `voiceover.mp3` + `source.mp4` + `script.txt`, output là MP4 1920×1080 đã render (`<slug>.mp4`) trong cùng folder. AUTOPILOT: scaffold → lint → render thẳng (KHÔNG preview) → gửi Telegram.
...
**AUTOPILOT (override):** Theo mục AUTOPILOT trong `agents/videoeditor/CLAUDE.md`. KHÔNG mở preview, KHÔNG dừng hỏi. Chạy 1 mạch tới MP4 1920×1080, render xong gửi về Telegram bằng `[SEND_FILE:...]`. Thiếu input thì tự chọn default.
...
Sau khi có MP4, in absolute path và gửi Telegram (file ≤ 50 MB):

```
Render xong: <slug>.mp4 (1920×1080, ~<duration>s)
[SEND_FILE:/absolute/path/<workspace>/<slug>.mp4|<slug> — keynote 16:9]
```
...
npx hyperframes render -q standard -o <slug>.mp4
# rồi gửi MP4 về Telegram: [SEND_FILE:/abs/<workspace>/<slug>.mp4|<slug> — keynote 16:9]
```
...
Sau Step 8 (render xong), print to user + gửi MP4:
