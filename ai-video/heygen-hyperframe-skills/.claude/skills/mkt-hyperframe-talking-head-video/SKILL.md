...
9. Render MP4 ────────── npx hyperframes render -q standard -o <slug>.mp4 (KHÔNG mở preview)
    │
    ▼
10. Gửi Telegram ─────── in path + [SEND_FILE:...<slug>.mp4]
```

**AUTOPILOT (override):** Theo `agents/videoeditor/CLAUDE.md` → mục AUTOPILOT. Skill chạy 1 mạch tới MP4, KHÔNG mở preview, KHÔNG dừng hỏi. Thiếu info thì tự chọn default (tỷ lệ mặc định 9:16). Render xong gửi MP4 về Telegram bằng `[SEND_FILE:...]`.
...
## Step 8: Render MP4 + gửi Telegram (AUTOPILOT — KHÔNG preview)
...
Nếu render báo lỗi lint → fix lỗi (xem Step 7 + `references/pitfalls.md`) rồi render lại. Lặp tới khi ra MP4.

Sau khi có MP4, in absolute path và gửi về Telegram bằng marker (file ≤ 50 MB):
...
Nếu MP4 > 50 MB: KHÔNG gửi file, chỉ báo path + size + lý do vượt giới hạn Telegram.
...
2. **AUTOPILOT render** — KHÔNG mở preview. Render thẳng `npx hyperframes render -q standard` ra MP4 rồi gửi Telegram. Xem mục AUTOPILOT trong `agents/videoeditor/CLAUDE.md`.
