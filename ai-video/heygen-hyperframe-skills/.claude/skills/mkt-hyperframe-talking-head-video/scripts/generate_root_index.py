...
def render(scenes, total_duration, header_label, footer_handle):
    ...
    # Cap the divider pulse loop so it ends before total_duration.
    # Each yoyo cycle is 1.2s; loop runs (repeat + 1) cycles from its start time.
    divider_loop_start = first_scene_start - 0.2
    divider_repeat = max(1, int((total_duration - divider_loop_start) / 1.2) - 1)
...
<body>
  <div data-composition-id="root" data-start="0" data-duration="{total_duration:.2f}" data-width="1080" data-height="1920">
...
{scenes_js_str}
      ...
      function goSplit(t) {{
        tl.to("#v-source", {{
          height: "50%", top: "50%",
          objectPosition: "center 48%",  // face sits in BOTTOM half — high value keeps it raised, not buried at the bottom edge
      ...
      // Divider pulse loop — repeat capped so the GSAP timeline never outruns
      // the voiceover (an uncapped loop stretches the rendered MP4 past total_duration)
      tl.to("#split-divider", {{
        scaleY: 1.4, duration: 1.2, ease: "sine.inOut",
        yoyo: true, repeat: {divider_repeat}, transformOrigin: "center center",
